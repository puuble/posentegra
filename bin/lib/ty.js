...
const { sendFormData, _asyncrequest, _providerrequest, getEnvironment, asyncForEach } = require("./helpers");
...
class TY {
  ...
  async findOrder(id) {
    ...
    var auth = "Basic " + Buffer.from(apiKey + ":" + apiSecretKey).toString("base64");
              ...
              if (sonuc.content.length > 0) {
                await asyncForEach(sonuc.content, async (data) => {
                  ...
                  if (process.env.LOG == true) {
                    fs.writeFileSync("./logs/ty-" + pid + ".json", JSON.stringify(body));
                  }
                });
              }
            }
